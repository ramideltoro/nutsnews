...
| `container-image.yml` | PR-required | Builds, smoke-tests, runs migration gates, publishes the immutable PR artifact, emits the current `Release candidate` check, and for trusted same-repository PRs runs the pre-merge VPS staging deploy, VPS staging UI smoke, Vercel staging deploy, Vercel staging UI smoke, Vercel production deploy, and Vercel production UI smoke stages. | Deploys only the trusted PR candidate to VPS staging, Vercel staging, and Vercel production before merge, with shared UI smoke evidence after each deployed target so far; main image publish is artifact work. |
