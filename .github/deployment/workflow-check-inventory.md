...
| `container-image.yml` | PR-required | Builds, smoke-tests, runs migration gates, and emits the current `Release candidate` check before merge; its main image publish is artifact work, not an environment deployment. | No deployment; PR release artifact work is handled by issue #296. |
