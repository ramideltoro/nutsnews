...
| `worker-uplift-admin-production-evidence.yml` | scheduled/operational | Runs only on protected operator dispatch to collect sanitized, read-only worker-uplift production admin evidence for an exact immutable deployment. | No deployment or production mutation; reads protected production credentials only through the Production environment and does not change ownership, ingestion, or database state. |
