...
| `supabase-standby-manifest-regression.yml` | PR-required | Guards the standby manifest validator for `supabase/standby_manifest.json`, schema fingerprint, table/view/sequence coverage, and database migration gate wiring before standby or migration automation changes merge. | No deployment. |
...
| `supabase-standby-readiness.yml` | manual recovery | Protected, typed-confirmation workflow for checking existing production Supabase hot-standby credential readiness and direct DB connectivity. | Preflight and protected readiness both run on `ubuntu-latest`; readiness sends the protected direct DB URL over stdin to the restricted backend forced-command SSH probe and requires the safe `READY` token. Reads protected standby credentials only on manual dispatch; no app deployment, no app/worker write enablement, no GitHub-side SQL or `psql`, and no row data export. |
...
| `worker-uplift-admin-production-evidence-contract.yml` | PR-required | Validates the read-only authenticated production admin evidence contract when its workflow, harness, tests, or documentation change. | No deployment. |
| `worker-uplift-admin-production-evidence.yml` | scheduled/operational | Runs an explicitly dispatched, read-only authenticated production admin evidence probe against an immutable deployed candidate. | No deployment; reads protected production credentials only through the Production environment. |
