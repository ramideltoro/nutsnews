...
| `supabase-standby` | `supabase-standby-readiness.yml` | Operator-only hot-standby credential readiness gate. It proves the fresh standby project inventory and direct DB connectivity without exposing standby credentials to normal app, worker, or release workflows. |
...
| --- | --- | --- |
| `NUTSNEWS_INFRA_STAGING_TOKEN` | `deploy-vps-staging`, `ui-smoke-vps-staging`, `staging-release.yml`, `vercel-production-release.yml` | Dispatches the immutable candidate to `ramideltoro/nutsnews-infra` with event `nutsnews-staging-release`, lets app-side VPS staging smoke wait for the protected infra qualification artifact, and lets Vercel production recovery verify that artifact includes passing admin backend operation smoke evidence before promotion. |
...
| `NUTSNEWS_STANDBY_SUPABASE_PROJECT_REF` | `supabase-standby-readiness.yml` | `supabase-standby` environment secret for the fresh standby project ref. It must differ from `NUTSNEWS_PRODUCTION_SUPABASE_PROJECT_REF`. |
| `NUTSNEWS_STANDBY_SUPABASE_URL` | `supabase-standby-readiness.yml` | `supabase-standby` environment secret for the standby project HTTPS URL. |
| `NUTSNEWS_STANDBY_SUPABASE_DB_URL` | `supabase-standby-readiness.yml` | `supabase-standby` environment secret for the direct standby Postgres URL. It must use the direct `db.<project-ref>.supabase.co:5432/postgres?sslmode=require` form. |
| `NUTSNEWS_STANDBY_SUPABASE_SERVICE_ROLE_KEY` | `supabase-standby-readiness.yml` | `supabase-standby` environment secret for future protected standby writes. Normal production app and worker traffic must not read it before an approved failover path exists. |
| `NUTSNEWS_STANDBY_SUPABASE_ANON_KEY` | `supabase-standby-readiness.yml` | `supabase-standby` environment secret for standby client-readiness validation. It is retained inside the protected environment until failover approval. |
...
| Verify Supabase hot-standby credential readiness | `supabase-standby-readiness.yml` | `workflow_dispatch` with `confirmation` set to `verify-supabase-standby-readiness` | Requires `NUTSNEWS_STANDBY_SUPABASE_PROJECT_REF`, `NUTSNEWS_STANDBY_SUPABASE_URL`, `NUTSNEWS_STANDBY_SUPABASE_DB_URL`, `NUTSNEWS_STANDBY_SUPABASE_SERVICE_ROLE_KEY`, and `NUTSNEWS_STANDBY_SUPABASE_ANON_KEY` in `supabase-standby`; emits only safe metadata and a read-only direct DB connectivity result |
