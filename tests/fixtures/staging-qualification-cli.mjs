...
async function fetchFixture(value, init = {}) {
  ...
  if (url.pathname === "/healthz") return json({ ok: true }, { ...security, "x-nutsnews-source-commit": commit, "x-nutsnews-build-id": "123456789-1" });
  ...
  if (url.pathname === "/api/runtime-config") return json({ sourceCommit: commit, buildId: "123456789-1", expectedImageDigest: digest, runtimeEnv: "staging", deploymentTarget: "vps-staging", configGeneration, sideEffectsMode: "disabled", telemetryEnabled: false }, { "cache-control": "no-store" });
