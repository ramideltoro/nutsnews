...
function validRuleset() {
  return [{
    name: "Require PRs and Merge Gate on main",
    target: "branch",
    enforcement: "active",
    bypass_actors: [],
...
for (const [message, mutate] of [
  ...
  ["Merge Gate context", (ruleset) => { ruleset[0].rules[3].parameters.required_status_checks = []; }],
  ["Release candidate context", (ruleset) => { ruleset[0].rules[3].parameters.required_status_checks.push({ context: "Release candidate", integration_id: 15368 }); }],
  ["Pre-merge deployment gate context", (ruleset) => { ruleset[0].rules[3].parameters.required_status_checks.push({ context: "Pre-merge deployment gate", integration_id: 15368 }); }],
  ["Merge Gate GitHub Actions integration", (ruleset) => { delete ruleset[0].rules[3].parameters.required_status_checks[0].integration_id; }],
