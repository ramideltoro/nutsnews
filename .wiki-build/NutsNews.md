<!-- Auto-generated from web/README.md. Do not edit this wiki page directly. -->
...
**A calm, mobile-first positive news platform powered by automation, AI curation, serverless infrastructure, CDN caching, centralized observability, and private admin dashboards.**
...
| Controller                    | Controller Worker triggers shard refreshes on a schedule                                         |
...
| AI Usage Dashboard            | Tracks OpenAI calls, tokens, cost, accepted/rejected decisions, and spike warnings               |
| Worker Shard Health Dashboard | Tracks shard freshness, feed count, fetch volume, run duration, stale shards, and warning shards |
...
The AI Usage dashboard lives at:
...
The Worker saves usage rows into Supabase so the admin dashboard can show exact run-level usage instead of relying only on rough estimates.

Important tracked fields include:
...
The goal of this dashboard is to show whether each Cloudflare Worker shard is healthy, stale, missing, slow, or needs attention.

This dashboard was added to make Worker sharding easier to operate as the platform grows.
...
```text
Which shard is stale?
Which shard has no feeds?
Which shard is not fetching articles?
Which shard is slow?
Which shard has save warnings?
Which shard has not reported a run yet?
```

### Shard status model

Shard status is calculated from saved Worker run telemetry.

Current statuses include:

| Status   | Meaning                                                                              |
| -------- | ------------------------------------------------------------------------------------ |
| Healthy  | The shard has a recent saved run and normal activity                                 |
| Warning  | The shard ran but has a warning, such as slow duration, zero fetches, or save issues |
| Stale    | The shard has not saved a run within the configured stale window                     |
| No Feeds | The shard has zero feeds assigned                                                    |
| Missing  | The dashboard has no saved run for that shard                                        |

### Stale shard timing

The stale threshold should match the controller schedule.

If the controller triggers **one shard every 5 minutes** and there are **25 shards**, then a full rotation takes:

```text
25 shards × 5 minutes = 125 minutes
```

A good stale threshold should be higher than the full rotation time.
...
That means a shard is marked stale only after it has not saved a run for about 3 hours.
...
A future improvement can add true failed-run tracking by storing failed Worker executions as first-class records. That would allow the dashboard to show:

* Latest failure message
* Error name
* Error details
* Consecutive failure count
* Failed status when the latest run failed
...
For example, with 25 shards and one shard triggered every 5 minutes, the controller completes a full rotation in about 125 minutes.
...
Each Worker shard runs the same source code. Only the shard configuration changes.
...
* Admin dashboards for operational health
...
The admin portal can continue expanding with dashboards for:
...
| Technology   | Purpose                                         |
| ------------ | ----------------------------------------------- |
| Next.js      | Public website, article pages, and admin portal |
| React        | Interactive feed and admin dashboard experience |
...
| Admin AI Usage Dashboard            | OpenAI usage and cost visibility               |
| Admin Worker Shard Health Dashboard | Worker shard health and stale shard visibility |
...
The public website and private admin portal.

It presents the mobile feed, article pages, SEO metadata, structured data, CDN-friendly responses, frontend error tracking, web application logs, and admin dashboards.

Current admin dashboards include:
...
It fetches RSS feeds, parses articles, filters unsuitable candidates, calls OpenAI for review, stores review decisions, publishes accepted articles, saves run telemetry, and logs structured activity.
...
It stores RSS feeds, AI review history, accepted articles, rejected articles, categories, summaries, timestamps, source links, and operational run data.
...
NutsNews uses a focused data model.
...
Stores run-level OpenAI usage and cost telemetry.

This powers the AI Usage dashboard.
...
The Worker Shard Health dashboard currently uses saved Worker telemetry to show shard freshness, shard health, and operational warnings.

This helps the operator understand whether shard processing is healthy without digging through logs.
...
That is why NutsNews uses several layers of observability.
...
> What is the operational health of the platform right now?
...
* Exposing AI usage through `/admin/ai-usage`
* Exposing shard health through `/admin/shards`
...
The architecture is modular, serverless, observable, and easy to extend.
...
* Private Google-protected admin portal
* AI Usage dashboard
* Worker Shard Health dashboard
...
* Add true failed Worker run tracking
* Add consecutive shard failure tracking
* Add latest Worker error message display
